...
import { Proposal, TokenMetadata, Vote, VoteDirection } from './types';
 ...
 */
export async function formatBidMessageText(id: number, bid: any) {
  const bidder = await resolveEnsOrFormatAddress(bid[0].sender.v);
  return `Gnar ${id} has received a bid of Ξ${ethers.utils.formatEther(bid[0].amount.v)} from ${bidder}`;
}
...
export function formatNewGovernanceProposalText(proposal: Proposal) {
  return `A new GnarsDAO proposal (#${proposal.id}) has been created: ${extractProposalTitle(
    proposal,
  )}`;
}
...
export async function getNounPngBuffer(tokenId: string): Promise<Buffer | undefined> {
  const dataURI = await tryF(() => nounsTokenContract.tokenURI(tokenId));
  if (isError(dataURI)) {
